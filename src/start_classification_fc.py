...
pathTensorBoardx = os.path.join(
    config["root"], config["tensoardboardx_folder"], timestr)
...
for epoch in range(config['epoch_num']):
    ...
    testId = random.randrange(0, len(inputsValidateSet))
    original = inputsValidateSet[testId]
    label = labelsValidateSetValue[testId]
    original.to(device)
